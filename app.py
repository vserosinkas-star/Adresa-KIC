...
def load_google_sheets():
    ...
    try:
        ...
        try:
            ...
            return None, f"Ошибка авторизации Google"
        ...
        # Определяем индексы колонок - ИСПРАВЛЕН СИНТАКСИС!
        col_index = {}
        ...
        for row in all_values[1:]:
            try:
                ...
                debug_log(f"Ошибка обработки строки: {e}")
                continue
        ...
        return result, f"Загружено {len(result)} записей из Google Sheets"
...
<body>
    ...
    <div class="box">
        ...
        <p><strong>Населенных пунктов в базе:</strong> ''' + str(len(data)) + '''</p>
    </div>
...
def handler(event, context=None):
    ...
    try:
        # Логируем полученный event
        debug_log(f"Event method: {event.get('httpMethod', 'GET')}")
        ...
        if method == 'GET':
            ...
            try:
                body = event.get('body', '{}')
                debug_log(f"Body type: {type(body)}, Body: {body[:200] if body else 'empty'}")
                ...
                debug_log(f"Telegram processing error: {e}")
                return {
                    'statusCode': 500,
                    'headers': {
                        'Content-Type': 'application/json'
